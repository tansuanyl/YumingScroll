import { deriveCharacterModelsFromStory, deriveSceneModelsFromStory } from "../data/demoProject";
import type { Project, ScriptScene, StoryState, StoryboardShot, VideoFlow, VisualPrompt } from "../types/domain";
...
export function normalizeProjectVideoFlows(project: Project): Project {
  const workflowEdges = project.workflowEdges || [];
  ...
  const missingFlows = storyState.storyboard
    .filter((shot) => !existingShotIds.has(shot.id))
    .map((shot) => createFlowFromShot(shot, existingFlowIds));
  const characterModels = project.characterModels.length > 0 ? project.characterModels : deriveCharacterModelsFromStory(storyState);
  const sceneModels = project.sceneModels.length > 0 ? project.sceneModels : deriveSceneModelsFromStory(storyState);

  if (
    storyState === project.storyState &&
    missingFlows.length === 0 &&
    project.workflowEdges === workflowEdges &&
    characterModels === project.characterModels &&
  ...
  return {
    ...
    characterModels,
    sceneModels,
    videoFlows: [...project.videoFlows, ...missingFlows]
  };
}

function syncStoryStateWithSeedanceSegments(storyState: StoryState): StoryState {
  ...
  for (let index = storyboard.length; index < segments.length; index += 1) {
    ...
    script.push(createScriptSceneFromSeedanceSegment(segment, shot, sceneId, index));
    changed = true;
  }
...
function createFlowFromShot(shot: StoryboardShot, existingFlowIds: Set<string>): VideoFlow {
  const id = createUniqueId(existingFlowIds, `flow-${shot.id}`);
  return {
    id,
    shotId: shot.id,
    nodes: {
...
function createStoryboardShotFromSeedanceSegment(
  ...
  const characterActions = cleanText(
  ...
  return {
    ...
    dialogue,
    imagePrompt,
    videoPrompt: segment.raw || `15 秒视频片段，${segment.title}，${characterActions}，${cameraMovement}`
  };
}

function createScriptSceneFromSeedanceSegment(segment: SeedanceSegment, shot: StoryboardShot, sceneId: string, index: number): ScriptScene {
  return {
    id: sceneId,
    title: segment.title || `第 ${index + 1} 段 15 秒`,
    location: shot.background,
    description: compactSegmentText(segment.body || segment.raw, 180) || shot.composition,
